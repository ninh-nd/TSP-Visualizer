...
public class DP implements Algorithm {
	...
	public DP(int start, double[][] distance) {
		...
		if (start < 0 || start >= N)
			throw new IllegalArgumentException("Invalid startMyNode.");
		if (N > 32)
...
//		  for (double[] row : distance) java.util.Arrays.fill(row, 10000);
	...
	public void run(Pane root) {
		...
		Algorithm.displayLine(root, finalGraph);
		String message = "Shortest tour: " + solver.getTour().toString() + "\nTour cost: "
				+ String.valueOf(solver.getTourCost());
